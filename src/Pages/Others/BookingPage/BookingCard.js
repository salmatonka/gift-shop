...
const BookingCard = ({b}) => {
    const {name,image,original_price,resale_price,rating} = b;
    return (
        ...
        <h3 className='text-xl font-semibold pt-4'>{name}</h3>
         ...
         {/* <Link to={`/booking/${_id}`}> <button type="button" className="w-60 py-3 font-semibold rounded hover:bg-red-400 bg-red-200 dark:text-gray-900">Book Now</button></Link> */}
    </div>
    );
};
