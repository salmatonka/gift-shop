...
const AddCards = ({p}) => {
    const {_id,name,image,original_price,resale_price,rating} = p;
    return (
        ...
        <h3 className='text-xl font-semibold pt-4 pb-8'>{name}</h3>
         ...
         <Link to={`/booking/${_id}`}> <button type="button" className="w-60 py-3 font-semibold rounded hover:bg-red-400 bg-red-200 dark:text-gray-900">Book Details</button></Link>
    </div>
